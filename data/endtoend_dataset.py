...
def get_transformB(convert=True):
    transform_list = []
    ...
    if convert:
        ...
        transform_list += [transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]
    return transforms.Compose(transform_list)
class EndtoendDataset(BaseDataset):
    """
    ...
    def __init__(self, opt):
        ...
        self.transform_B = get_transformB()
    ...
    def __getitem__(self, index):
        ...
        C_img = m.open(C_path).convert('L')

        A, C = transform(A_img, C_img, self.opt)
        B = self.transform_B(B_img)

        return {'A': A, 'B': B, 'C': C,  'A_paths': A_path, 'B_paths': B_path, 'C_path': C_path}
    def __len__(self):
