...
if __name__ == '__main__':
    ...
    # 创建训练模型
    model_train = create_model(opt)
    model_train.train()
    opt.continue_train = True
    model_train.setup(opt)

    # 创建验证模型
    opt.isTrain = False
    model_val = create_model(opt)
    model_val.eval()
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for i, data in enumerate(dataset_train):
            ...
            gt = np.squeeze(data["C"].numpy(), axis = 1)  # [N, W, H]
        ...
        for i, data in enumerate(dataset_val):
            ...
            if i % opt.display_freq == 0:
                save_segment_result(model_val.get_current_visuals(), epoch, opt.display_winsize, image_dir, web_dir, opt.name)
